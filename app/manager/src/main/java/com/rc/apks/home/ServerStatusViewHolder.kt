...
import com.rc.apks.databinding.HomeServerStatusBinding
...
class ServerStatusViewHolder(private val binding: HomeServerStatusBinding, root: View) :
    ...
    override fun onBind() {
        ...
        // Accessibility: combine title and summary for screen readers
        val accessibilityText = buildString {
            append(title.replace("<[^>]*>".toRegex(), ""))
            if (summary.isNotEmpty()) {
                append(". ")
                append(summary.replace("<[^>]*>".toRegex(), ""))
            }
        }
